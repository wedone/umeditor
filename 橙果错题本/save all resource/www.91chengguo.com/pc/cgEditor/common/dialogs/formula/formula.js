    ...
    UM.registerWidget('formula', {
        ...
        initContent: function (_editor, $widget) {
            ...
            $.each(formula, function (k, v) {
                var contentHtml = [];
                $.each(v, function (i, f) {
                    //2019.6.12 屏蔽 > < 符号(原因（<选择后展示不了，>选择后保存为图片不对）)
                    var shieldStyle = ""
                    if(f == '>' || f == '<'){
                        shieldStyle = 'pointer-events: none;background: #8c8c8c45';
                    }
                    //end
                    contentHtml.push('<li class="edui-formula-latex-item" data-latex="' + f + '" style="background-position:-' + (xMax * 30) + 'px -' + (yMax * 30) + 'px;' + shieldStyle + ' "></li>');
                    if (++xMax >=8) {
                        ++yMax; xMax = 0;
                    }
                });
